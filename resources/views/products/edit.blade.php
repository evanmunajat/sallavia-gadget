                        ...
                        @else
                            <div class="flex gap-2">
                                <input type="text" name="specifications[0][key]" placeholder="Nama spesifikasi"
                                       class="w-1/2 border-gray-300 rounded-lg px-2 py-1">
                                <input type="text" name="specifications[0][value]" placeholder="Nilai"
                                       class="w-1/2 border-gray-300 rounded-lg px-2 py-1">
                                <button type="button" onclick="removeSpec(this)"
                                        class="text-red-500 font-bold">×</button>
                            </div>
                            ...
                            @foreach($product->images as $img)
                                <div class="relative flex flex-col items-center" data-id="{{ $img->id }}">
                                    <img src="{{ asset('storage/'.$img->image) }}"
                                         class="w-full h-32 object-cover rounded-lg shadow mb-2">
                                    <button type="button"
    ...
    <script>
        ...
        function addSpec() {
            ...
            div.innerHTML = `
                ...
                <input type="text" name="specifications[${index}][value]" placeholder="Nilai" class="w-1/2 border-gray-300 rounded-lg px-2 py-1">
        ...
        function deleteImage(imageId, el) {
            ...
            .then(data => {
                if(data.success){
                    const parentDiv = el.closest('.relative');
